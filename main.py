...
def cadastro():
    ...
    if subId is not None:
        # Verifica se o e-mail já está cadastrado com o subId
        cursor.execute("SELECT * FROM usuarios WHERE email = %s AND subId = %s", (emailcad, subId))
        ...
        if existing_user:
            ...
            try:
                # Insere novo usuário
                cursor.execute("""
                    INSERT INTO usuarios (nome, sobrenome, email, senha, subId)
                    VALUES (%s, %s, %s, %s, %s)
        ...
        cursor.execute("SELECT * FROM usuarios WHERE email = %s", (emailcad,))
        existing_user = cursor.fetchone()
